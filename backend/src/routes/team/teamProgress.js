  ...
  try {
    ...
    if (open_code === expectedOpenCode) {
      ...
      const result = await db.query(
        'UPDATE team_routes SET open_code_verified = TRUE WHERE stage_id = $1 AND team_id = $2 RETURNING *',
...
router.get('/get-hint', teamAuth, async (req, res) => {
  ...
  try {
    ...
    if (secondsSinceAssigned < sixMinutes) {
      const remaining = Math.ceil(sixMinutes - secondsSinceAssigned);
      return res.status(400).json({
        success: false,
        message: 'Hint not available yet.',
        hintCountdown: remaining
      });
    }
    const questionResult = await db.query(
    ...
    return res.json({
      ...
      hint: {
        hint1: hint1 ? hint1.toString('base64') : null,
        hint2: hint2 ? hint2.toString('base64') : null
      }
    });
  } catch (error) {
    console.error("Error getting hint:", error);
    return res.status(500).json({ error: 'Server error while fetching hint.' });
  }
});
